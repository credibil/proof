//! Implementation for the Identity Overlay Network (ION) DID method. Anchors public DID documents
//! to the Bitcoin ledger.
    ...
    K: KeyRing + Signer,
{
    ...
    pub fn new(
        challenge_url: &str, solution_url: &str, resolution_url: &str, keyring: K, anchor: bool,
        network: Option<String>, controller: Option<String>,
    ) -> Self {
    ...
    pub(crate) async fn submit(&self, req: &Request) -> Result<()> {
        ...
        headers.insert(
            ...
            match reqwest::header::HeaderValue::from_str(&pow.challenge_nonce) {
                Ok(v) => v,
                Err(e) => {
                    tracerr!(Err::InvalidFormat, "failed to set challenge nonce header: {}", e)
                }
            },
        );
        headers.insert(
            ...
            match reqwest::header::HeaderValue::from_str(&pow.answer_nonce) {
                Ok(v) => v,
                Err(e) => tracerr!(Err::InvalidFormat, "failed to set answer nonce header: {}", e),
            },
        );
    ...
    async fn proof_of_work(&self, req: &Request) -> Result<ProofOfWork> {
        ...
        let pow = loop {
            ...
            if Utc::now() > expires {
                tracerr!(Err::Expired, "challenge expired before proof of work nonce was found.");
            }
    ...
    pub(crate) async fn resolve_did(&self, did: &str) -> Result<Resolution> {
        ...
        let res = match self.http_client.get(url).send().await {
            Ok(res) => res,
            Err(e) => tracerr!(Err::RequestError, "failed to call DID resolution endpoint: {}", e),
        };
    ...
    pub(crate) fn create_request(
        recovery_commitment: &str, update_commitment: &str, doc: &DidDocument,
    ) -> Result<Request> {
        let delta = Delta {
            patches: vec![Patch {
    ...
    pub(crate) fn long_did(&self, req: &Request) -> Result<String> {
        if req.type_ != OperationType::Create {
            tracerr!(Err::InvalidInput, "DID construction requires a create request");
        }
    ...
    T: serde::de::DeserializeOwned,
{
    if res.status().is_success() {
        ...
        match res.json::<ErrorResponse>().await {
            ...
            Err(e) => {
                tracerr!(Err::DeserializationError, "failed to deserialize error response: {}", e)
            }
        }
    }
}
