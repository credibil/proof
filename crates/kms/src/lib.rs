...
use credibil_ecc::{Algorithm, Curve, Signer};
...
impl Keyring {
    ...
    pub async fn add_key(&mut self, id: impl Into<String>) -> anyhow::Result<()> {
    ...
    pub async fn jwk(&mut self, id: impl Into<String>) -> anyhow::Result<PublicKeyJwk> {
        let id = id.into();
        let vk = match self.keys.verifying_key(&id).await {
            Ok(vk) => vk,
            Err(_) => {
                self.add_key(id.clone()).await?;
                self.keys.verifying_key(&id).await?
            }
        };
    ...
    pub async fn multibase(&mut self, id: impl Into<String>) -> anyhow::Result<String> {
    ...
    // errors occur.
    pub async fn next_jwk(&self, id: impl Into<String>) -> anyhow::Result<PublicKeyJwk> {
    ...
    // occur.
    pub async fn next_multibase(&self, id: impl Into<String>) -> anyhow::Result<String> {
...
impl Signature for Keyring {
    async fn verification_method(&self) -> anyhow::Result<VerifyBy> {
